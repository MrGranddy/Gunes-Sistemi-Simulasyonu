...
class Gokcismi:
    ...
    def tum_cisimlere_kuvvet_etki():
        kopya_cisimler = list(cisimler)
        for i in range(len(cisimler)):
            cisimler[i].hizi_degistir(kopya_cisimler)

    def tum_cisimleri_hareket_ettir():
        for i in range(len(cisimler)):
            cisimler[i].hizi_uygula()

    def tum_cisimleri_cizdir():
        for i in range(len(cisimler)):
            yeni_koordinatlar = (cisimler[i].koordinatlar[
                                 0] / oran, cisimler[i].koordinatlar[1] / oran)
            yeni_koordinatlar = (
                int(yeni_koordinatlar[0] + en / 2), int(boy / 2 - yeni_koordinatlar[1]))
            yeni_cap = int(cisimler[i].cap / oran)
            pygame.draw.circle(
...
cisimler += [gunes, dunya, jupiter, mars, venus, merkur, saturn]

running = True
while running:
    Gokcismi.tum_cisimleri_hareket_ettir()
    Gokcismi.tum_cisimlere_kuvvet_etki()
    Gokcismi.tum_cisimleri_cizdir()
